...
import { fallbackSafetyResponse } from "./safety-rules";
...
export type ChatPlan = {
  ...
  calendarAction?: CalendarActionPlan | null;
};
...
function buildContextText(input: ChatAgentInput) {
  return [
...
function fallbackIntent(input: ChatAgentInput): ChatIntent {
  const text = [
    input.query,
    input.pageTitle,
    input.visibleText,
...
function fallbackPlan(input: ChatAgentInput, intent: ChatIntent): ChatPlan {
  const baseContext = buildContextText(input);
  if (intent === "page_security") {
    const safety = fallbackSafetyResponse(baseContext, "scam_check", input.url);
    return {
...
export async function classifyChatIntent(input: ChatAgentInput): Promise<ChatClassification> {
  const prompt = [
  ...
  try {
    ...
    const parsed = safeParseJson<Partial<ChatClassification>>(rawText);
    if (parsed?.intent) {
      return {
...
export async function generateChatPlan(
  input: ChatAgentInput & { intent: ChatIntent },
): Promise<ChatPlan> {
  const prompt = [
  ...
  try {
    const rawText = await runPrompt(prompt);
    const parsed = safeParseJson<Partial<ChatPlan>>(rawText);
    if (parsed?.message && parsed?.summary && parsed?.nextStep && parsed?.explanation) {
      return {
        ...
        suspiciousSignals: Array.isArray(parsed.suspiciousSignals)
          ? parsed.suspiciousSignals.filter((item): item is string => typeof item === "string")
          : [],
